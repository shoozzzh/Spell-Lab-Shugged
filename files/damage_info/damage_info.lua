...
if #highest_dps > 0 then
	GuiTextCentered( gui, x_pos, 0, get_text( "damage_info_dps" ) .. highest_dps )
end

local total_damage = GlobalsGetValue( "spell_lab_shugged_recent_total_damage", "" )
if #total_damage > 0 then
	GuiTextCentered( gui, x_pos, 0, get_text( "damage_info_total_damage" ) .. total_damage )
end
...
GuiTextCentered( gui, x_pos, 0, get_text( "damage_info_total_projectile_damage" ) .. separator( math.floor( total_projectile_damage * 25 + 0.5 ) ) )
GuiTextCentered( gui, x_pos, 0, get_text( "damage_info_total_projectiles_num" ) .. separator( math.floor( total_projectiles ) ) )