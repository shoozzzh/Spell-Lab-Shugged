...
function gui_elements.scroll_table( x, y, cell_2dlist, cell_gui_fn, cell_width, cell_height )
	...
	width = cell_width * width - 2
	local height = math.min( #cell_2dlist * cell_height )

	pop.scroll_box_begin( x, y, width, height )

	pop.matrix( x, y, cell_2dlist, cell_width, cell_height, cell_gui_fn )
...
function gui_elements.button_setting_toggle( sprite_filename, key, name, description )
	local value = ModSettingGet( mod_setting_prefix .. key ) == true
	if not value then
